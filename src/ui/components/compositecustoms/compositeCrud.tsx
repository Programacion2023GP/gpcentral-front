...
const FieldWrapper = ({
   ...
   const handleSelectChange = useCallback(
      (selectedItem: any) => {
         ...
         if (field.onChange)
            field.onChange(selectedItem, formik, actionsDispatch);
   ...
   switch (field.typeField) {
         ...
         return (
            <FormikDatePicker
               ...
               onChange={(value, formik) => {
                  if (field.onChange)
                     field.onChange(value, formik, actionsDispatch);
               }}
            />
         );
      ...
      case "DateRange":
         ...
         return (
            <FormikDateRange
               nameFrom={
                  field.dateRangeConfig?.nameFrom || field.name + "_from"
               }
            ...
            if (
               ...
               const boxes = (definition as BoxGroup<TForm>[]).map((box) => ({
                  ...
                  fields: computedFields.filter((field) =>
                     box.fields.includes(field.name as NestedKeys<TForm>),
                  ),
               }));
   ...
   return (
      <div
         ...
         <CompositePage
            ...
            onCancel={
               sectioned.hasSections ? undefined : () => hook.setOpen(false)
            }
               ...
               return (
                  <FormikForm
                     ...
                     onSubmit={async (values) => {
                        try {
                           if (hasFileFields)
                              await hook.postItem(values as TForm, true);
                           else await hook.postItem(values as TForm);
                           hook.setOpen(false);
                           Swal.fire({
                              icon: "success",
