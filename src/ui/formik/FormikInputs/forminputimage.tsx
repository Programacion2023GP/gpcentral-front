...
const FILE_PRESETS: Record<FilePreset, FileTypeConfig> = {
   ...
   code: {
      accept:
         "text/javascript,text/typescript,text/html,text/css,application/json,text/plain",
      extensions: [
         "js",
         "ts",
         "jsx",
...
export const FormikFileInput: React.FC<FormikFileInputProps> = ({
   ...
   const allExtensions = customExtensions
      ? [
           ...new Set([
              ...
              ...customExtensions.map((e) => e.replace(".", "")),
           ]),
        ]
      : resolvedConfig.extensions;
   ...
   const errorMsg =
      touched && formik?.errors?.[name] ? String(formik.errors[name]) : null;
   ...
   useEffect(() => {
      ...
      if (currentKey !== newKey) {
         entries.forEach((e) => {
            if (e.preview && e.preview.startsWith("blob:"))
      ...
      (newEntries: FileEntry[]) => {
         ...
         if (effectiveMultiple) {
            newFormikValue = newEntries.map((entry) => {
               ...
               return (
                  entry.existingUrl || entry.existingData || entry.file.name
               );
            });
         } else {
         ...
         if (
            JSON.stringify(currentFormikValue) !==
            JSON.stringify(newFormikValue)
         ) {
   ...
   const processFiles = useCallback(
      async (files: File[]) => {
         ...
         const slots = effectiveMultiple
            ? maxFiles - entries.filter((e) => e.status !== "error").length
            : 1;
         ...
         setEntries((prev) => {
            const updated = effectiveMultiple
               ? [...prev, ...newEntries]
               : newEntries;
            return updated;
         });
                     ...
                     prev.map((e) =>
                        e.id === entry.id
                           ? {
                                ...
                                status: prog >= 100 ? "success" : "loading",
                             }
                           : e,
                     ),
                  );
   ...
   const canAddMore = effectiveMultiple
      ? entries.filter((e) => e.status !== "error").length < maxFiles
      : entries.length === 0;
   ...
   // ──────────────────────────────────────────────────────────────
   return (
      <ColComponent responsive={responsive} autoPadding={true}>
                        ...
                        <div
                           ...
                           {allExtensions.map((ext) => (
                              <ExtBadge
                                 ...
                                 colorLight={resolvedConfig.colorLight}
                              />
                           ))}
                        </div>
                        <div
                           ...
                           {compressImages && (
                              <span style={{ fontSize: 12, color: DS.text3 }}>
                                 🖼️ Compresión activa (máx. {imageMaxWidth}x
                                 {imageMaxHeight})
                              </span>
                           )}
                        </div>
               ...
               <div
                  ...
                  style={{
                     ...
                     boxShadow: isDragging
                        ? `0 0 0 3px ${resolvedConfig.color}25`
                        : "none",
                     boxSizing: "border-box",
                  }}
                  onMouseEnter={(e) => {
                  ...
                  onMouseLeave={(e) => {
                     if (!isDragging) {
                        e.currentTarget.style.borderColor = errorMsg
                           ? DS.borderError
                           : DS.border;
                        e.currentTarget.style.background = errorMsg
                           ? DS.errorBg
                           : DS.white;
                     }
                  }}>
