...
// ==================== HOOK AVANZADO ====================
export interface AdvancedCrudReturn<
   TForm = any,
   TTable = any,
> extends GenericDataReturn<TForm> {
   // Búsqueda y filtros
   search: string;
...
>(
   ...
   const bulkUpdate = useCallback(
      async (data: Partial<TForm>, ids?: (string | number)[]) => {
   ...
   const bulkAction = useCallback(
      async (actionId: string) => {
         const action = advancedConfig?.bulkActions?.find(
            (a) => a.id === actionId,
         );
         if (!action) return;
   ...
   useEffect(() => {
      ...
      const connectWebSocket = () => {
         ...
         const wsUrl =
            advancedConfig.realtime.endpoint ||
         ...
         ws.onmessage = (event) => {
            try {
               ...
               if (
                  type === "CREATE" &&
                  advancedConfig.realtime.events.includes("create")
               ) {
                  baseHook.setItems((prev) => [...prev, payload]);
               } else if (
                  type === "UPDATE" &&
                  advancedConfig.realtime.events.includes("update")
               ) {
                  ...
                  type === "DELETE" &&
                  advancedConfig.realtime.events.includes("delete")
               ) {
                  baseHook.setItems((prev) =>
         ...
         ws.onclose = () => {
            ...
            const maxAttempts =
               advancedConfig.realtime.maxReconnectAttempts || 5;
            if (reconnectAttemptsRef.current < maxAttempts) {
               reconnectAttemptsRef.current++;
               const interval =
                  advancedConfig.realtime.reconnectInterval || 3000;
   ...
   const exportToExcel = useCallback(
      (customData?: TTable[]) => {
         ...
         XLSX.writeFile(
            workbook,
            `${config.prefix}_export_${new Date().toISOString().split("T")[0]}.xlsx`,
         );
      },
      [filteredItems, config.prefix],
   );
   ...
   const refreshStats = useCallback(() => {
      ...
      const stats: DashboardStats<TTable> = {
         total: items.length,
         active: items.filter((item) => (item as any).status === "active")
            .length,
         inactive: items.filter((item) => (item as any).status === "inactive")
            .length,
         recentActivity: [],
         groupBy: {},
         trends: [],
   ...
   const fetchRelation = useCallback(
      async (field: string) => {
         const relation = advancedConfig?.relations?.find(
            (r) => r.field === field,
         );
         if (!relation) return;
   ...
   const loadRelationOptions = useCallback(
      async (field: string): Promise<any[]> => {
